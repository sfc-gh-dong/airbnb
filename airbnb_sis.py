...
def get_data():
    df = session.table("airbnb.public.airbnb")
    return df
...
def test_sql(q):
    try:
        session.sql("select * from airbnb.public.airbnb where " +q +" limit 1").collect()
        return True
...
st.title("🏖️ AirBnB LLM Chat Search")
...
df = get_data()
df = df.select("NAME","CLEANLINESS_R","ACCURACY_R","COMMUNICATION_R","LOCATION_R","CHECKIN_R","VALUE_R","PRICE","GUESTS","DESCRIPTION","REVIEWS","SUMMARY","SENTIMENT","IMAGE","LAT","LON")
df = df.to_pandas()
...
with st.expander("Preview data of selected fields."):
    ...
    st.dataframe(sample[["NAME","PRICE","GUESTS","REVIEWS","SUMMARY"]].sample(5),use_container_width=True)
...
with st.expander("Have a look at our AirBnB locations worldwide, with added summarization and sentiment."):
    st.pydeck_chart(pdk.Deck(
        ...
        tooltip={"html": "<img src='{IMAGE}' height='120' width='auto'></img>\
                          <b><u>{NAME}</u></b><br/>\
                          ...
                          <b>Sentiment : </b>{SENTIMENT}<br/>", "style": {"color": "white"}},
    ))
    ...
    with st.chat_message("assistant"):
        ...
        if decomposed is not None:
            ...
            if q1_phrase != "" and q2_phrase != "":
                q1embed = "select *, VECTOR_L2_DISTANCE(embedding, snowflake.cortex.embed_text('e5-base-v2', '{llm}')) as l2 from airbnb.public.airbnb \
            ...
            # only llm
                ...
                q1embed = "select *, VECTOR_L2_DISTANCE(embedding, snowflake.cortex.embed_text('e5-base-v2', '{llm}')) as l2 from airbnb.public.airbnb order by l2 asc limit 10"
            ...
            # only sql
                ...
                q1embed = "select * from airbnb.public.airbnb where {where} limit 50"
            ...
            if q!="":
                ...
                if not df.empty:
                    ...
                    st.pydeck_chart(pdk.Deck(
                        ...
                        tooltip={"html": "<img src='{IMAGE}' height='120' width='auto'></img><br/>\
                                <b><u>{NAME}</u></b><br/>\
